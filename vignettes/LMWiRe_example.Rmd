...
The UCH dataset stands for Urine-Citrate-Hippurate and comes from \textit{Rousseau et al.} It contains 3 elements : a matrix `responses` with 34 observations of 600 response variables representing the spectra from the H-NMR spectroscopy, a formula for the GLM model used and `design` matrix with 34 observations and 5 explanatory variables.

## Spectrum Visualization (= responses)

The 600 response variables are the spectrum of the H-NMR for each observations. 

```{r Spectrum visualization}

LinePlot(UCH$responses[1:2,],main="H-NMR spectrum")
...
```{r PCA}
ResPCA = SVDforPCA(UCH$responses)
```
...
ResLMEffectMatrices = LMEffectMatrices(ResLMModelMatrix,UCH$responses)
